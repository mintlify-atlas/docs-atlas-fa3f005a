...
Sys.Witch V2 is a React SPA. Each `.html` file (`about.html`, `projects.html`, `skills.html`, etc.) is an identical static shell — the same `<div id="root"></div>` target that loads the same compiled React bundle from `assets/main.js`. None of those HTML files contain any unique markup.
...
<Info>
  The `assets/` directory is a Vite build output — do not edit files inside it directly. Your source is in `components/` and `data/`.
</Info>
