...
Import the `Sigil` component and pass it an `id` from the library above. The `size`, `color`, and `className` props are all optional.

| Prop | Type | Required | Description |
|---|---|---|---|
| `id` | `string` | Yes | Sigil ID from the library above |
| `size` | `number` | No | Render size in pixels (default inferred from SVG viewBox) |
| `color` | `string` | No | One of: `purple`, `magenta`, `cyan`, `lime` — maps to the matching neon color token |
...
<Tip>
  Sigil paths use a 24×24 viewBox coordinate space. Keep your custom path values within 0–24 on both axes for consistent sizing alongside the built-in library.
</Tip>
